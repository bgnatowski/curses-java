...
public class EdgeTariffElectricityTest {
    ...
    private final Integer kwhUsedInCase;
    private final boolean isTariffInCase;
    private final float sumKwhInCase;

    private final float sumKwhTariffInCase;

    public EdgeTariffElectricityTest(Integer kwhUsedInCase, boolean isTariffInCase, float sumKwhInCase, float sumKwhTariffInCase) {
    ...
    public void givenDependOnTariffWhenKwhAdditionThenSpecificCounterIsIncreased() {
        //Given
        // setTariff Time mock iskTariffNow() !
        Mockito.when(tp.isTariffNow()).thenReturn(isTariffInCase);
        //When
        electricityMeter.addKwh(kwhUsedInCase);
        //Then
        if (isTariffInCase)
            assertEquals(sumKwhInCase, electricityMeter.getKwh(), 0.01);
        else
            assertEquals(sumKwhTariffInCase, electricityMeter.getKwhTariff(), 0.01);
    }
